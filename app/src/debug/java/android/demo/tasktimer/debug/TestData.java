package android.demo.tasktimer.debug;

import android.content.ContentResolver;
import android.content.ContentValues;
import android.database.Cursor;
import android.demo.tasktimer.TasksContract;
import android.demo.tasktimer.TimingsContract;
    ...
    public static void generateTestData(ContentResolver contentResolver){
        ...
        if ((cursor!=null) && (cursor.moveToFirst())) {
            do {
                ...
                //generate between lower bound and upper bound random timings for this task
                int loopCount = LOWER_BOUND + getRandomInt(UPPER_BOUND - LOWER_BOUND);

                for (int i = 0; i< loopCount;i ++) {
                    long randomDate = randomDateTime();

                    //generate a random duration between  and hours
